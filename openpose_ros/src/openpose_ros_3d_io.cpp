...
openpose_ros_msgs::PointWithProb3D OpenPoseROSIO::get3D_cloud(float x_pixel, float y_pixel, float prob)
{

    int x = 0, y = 0;
    openpose_ros_msgs::PointWithProb3D bodypart_depth;
    std::vector<unsigned long long int> x_indices, y_indices;
    ...
    {
		auto pt_3d = cloud_ptr_->at(x_indices[n], y_indices[n]);

    	if (not std::isnan(pt_3d.z) && not std::isinf(pt_3d.z))
    	{
      		if (pt_3d.x <= pcl_max.x &&  pt_3d.x >= pcl_min.x)
        	{  
	  			if (pt_3d.y <= pcl_max.y && pt_3d.y >= pcl_min.y)
	   			{
	    			if (pt_3d.z <= pcl_max.z && pt_3d.z >= pcl_min.z)
	    			{
						possible_x.push_back(pt_3d.x);
				  	    possible_y.push_back(pt_3d.y);
				  	    possible_z.push_back(pt_3d.z);
				    }
			    }
	        }
      	}
